/**
 * 
 * Forked from https://gist.githubusercontent.com/Hyseen/b06e911a41036ebc36acf04ddebe7b9a/raw/nf_check.js?version=1633090677159
 * Translated into English for ease of use
 * 
 * [Panel]
 * nf_check = script-name=nf_check, title="Netflix Check", content="Please Reload", update-interval=1
 *
 * [Script]
 * nf_check = type=generic, script-path=https://raw.githubusercontent.com/dykderrick/Network-Proxy-Rules/derrick/Surge/Scripts/nf_check.js, argument=title=Netflix Check
 *
 * Support self-configured arguments. Example: argument=key1=URLEncode(value1)&key2=URLEncode(value2). Details as below.
 * title: Panel title
 * fullContent: text displaying when fully supported. #REGION_FLAG#, #REGION_CODE#, #REGION_NAME#, #REGION_NAME_EN#
 * fullIcon: Icon displaying when full supported. Valid SF Symbol Name only.
 * fullIconColor: Icon's color when fully supported. HEX code.
 * fullStyle: Icon type when fully supported.. Values are good, info, alert, error.
 * onlyOriginalContent: text displaying when NF only. 
 * onlyOriginalIcon: Icon displaying when NF only.
 * onlyOriginalIconColor: Icon's color when NF only.
 * onlyOriginalStyle: Icon type when NF only.
 * notAvailableContent: text displaying when NF is unavailable.
 * notAvailableIcon: icon displaying when NF is unavailable.
 * notAvailableIconColor: icon color when NF is unavailable.
 * notAvailableStyle: icon type when NF is unavailable.
 * errorContent: text displaying when error checking.
 * errorIcon: icon displaying when error checking,
 * errorIconColor: icon color when error checking.
 * errorStyle: icon type when error checking.
 * 
 */
